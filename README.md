...
一個結合 **Gemma3 多模態大語言模型 (Vision-Language Model, VLM)** 與 **OCR（例如 PaddleOCR）** 的貨物驗收輔助工具。  
協助倉庫／採購／工程人員，從 **收貨單影像 (invoice / delivery note image)** 自動整理出結構化 JSON，並與內部採購料表進行 **模糊比對 (fuzzy matching)**。
...
- 目標：  
  把「收貨單影像 → OCR 輸出 → 結構化 JSON → 料表比對」做成一條自動化 pipeline（處理流程），  
  減少人工對帳、抄寫與比對錯誤。
